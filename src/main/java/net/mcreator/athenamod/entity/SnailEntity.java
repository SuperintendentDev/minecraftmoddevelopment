...
public class SnailEntity extends Monster implements GeoEntity {
	...
	public SnailEntity(PlayMessages.SpawnEntity packet, Level world) {
		this(AthenaModModEntities.SNAIL.get(), world);
	}
	...
	protected void dropCustomDeathLoot(DamageSource source, int looting, boolean recentlyHitIn) {
		...
		this.spawnAtLocation(new ItemStack(AthenaModModItems.SNAIL_SHELL.get()));
	}
	...
	public static void init() {
		SpawnPlacements.register(AthenaModModEntities.SNAIL.get(), SpawnPlacements.Type.ON_GROUND, Heightmap.Types.MOTION_BLOCKING_NO_LEAVES,
	...
	private PlayState movementPredicate(AnimationState event) {
		if (this.animationprocedure.equals("empty")) {
			if ((event.isMoving() || !(event.getLimbSwingAmount() > -0.15F && event.getLimbSwingAmount() < 0.15F))
				...
				return event.setAndContinue(RawAnimation.begin().thenLoop("snailwalk"));
			}
			return event.setAndContinue(RawAnimation.begin().thenLoop("snailidle"));
		}
