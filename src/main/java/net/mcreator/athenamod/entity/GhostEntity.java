...
import net.mcreator.athenamod.procedures.GhostRightClickedOnEntityProcedure;
...
public class GhostEntity extends Monster implements GeoEntity {
	...
	public GhostEntity(PlayMessages.SpawnEntity packet, Level world) {
		this(AthenaModModEntities.GHOST.get(), world);
	}
	...
	protected void defineSynchedData() {
		...
		this.entityData.define(TEXTURE, "ghosttexture");
	}
	...
	public SoundEvent getAmbientSound() {
		return ForgeRegistries.SOUND_EVENTS.getValue(new ResourceLocation("athena_mod:ghostamb"));
	}
	...
	public InteractionResult mobInteract(Player sourceentity, InteractionHand hand) {
		...
		GhostRightClickedOnEntityProcedure.execute(entity, sourceentity);
		return retval;
	}
	...
	public static void init() {
		SpawnPlacements.register(AthenaModModEntities.GHOST.get(), SpawnPlacements.Type.ON_GROUND, Heightmap.Types.MOTION_BLOCKING_NO_LEAVES,
	...
	private PlayState movementPredicate(AnimationState event) {
		if (this.animationprocedure.equals("empty")) {
			if ((event.isMoving() || !(event.getLimbSwingAmount() > -0.15F && event.getLimbSwingAmount() < 0.15F)) && this.onGround()) {
				return event.setAndContinue(RawAnimation.begin().thenLoop("ghostidle"));
			}
			if (!this.onGround()) {
				return event.setAndContinue(RawAnimation.begin().thenLoop("ghostidle"));
			}
