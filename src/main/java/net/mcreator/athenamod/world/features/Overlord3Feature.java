...
public class Overlord3Feature extends Feature<NoneFeatureConfiguration> {
	...
	public Overlord3Feature() {
		super(NoneFeatureConfiguration.CODEC);
		base_blocks = List.of(AthenaModModBlocks.HEAVEN_STONE.get());
	}
	...
	public boolean place(FeaturePlaceContext<NoneFeatureConfiguration> context) {
		if (!generate_dimensions.contains(context.level().getLevel().dimension()))
			return false;
		if (template == null)
			template = context.level().getLevel().getStructureManager().getOrCreate(new ResourceLocation("athena_mod", "overlordore3"));
		if (template == null)
			return false;
