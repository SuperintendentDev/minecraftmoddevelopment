...
public class DragonDungeonFeature extends Feature<NoneFeatureConfiguration> {
	private final Set<ResourceKey<Level>> generate_dimensions = Set.of(Level.OVERWORLD);
	private StructureTemplate template = null;

	public DragonDungeonFeature() {
	...
	public boolean place(FeaturePlaceContext<NoneFeatureConfiguration> context) {
		if (!generate_dimensions.contains(context.level().getLevel().dimension()))
			return false;
		if (template == null)
			template = context.level().getLevel().getStructureManager().getOrCreate(new ResourceLocation("athena_mod", "dragondungeon"));
		if (template == null)
			return false;
		...
		if ((context.random().nextInt(1000000) + 1) <= 300) {
			...
			for (int a = 0; a < count; a++) {
				...
				BlockPos spawnTo = new BlockPos(i + 0, j + -41, k + 0);
